import { Prisma } from '@prisma/client';
import { toTimezone } from '../generic/generic.utils';

export const baseExtension = Prisma.defineExtension((client) => {
  return client.$extends({
    query: {
      $allModels: {
        async $allOperations({ model, operation, args, query }) {
          const anyArgs = args as any;
          const tz = anyArgs?.where?.tz || 'Africa/Lagos';
          if (anyArgs?.where?.tz) {
            delete anyArgs.where.tz;
          }

          // Soft delete filter for read operations
          if (['findFirst', 'findMany', 'findUnique', 'count', 'aggregate', 'groupBy'].includes(operation)) {
            if (anyArgs.where) {
              if (anyArgs.where.deleted_at === undefined) {
                anyArgs.where.deleted_at = null;
              }
            } else {
              anyArgs.where = { deleted_at: null };
            }
          }

          // Default pagination and sort for findMany
          if (operation === 'findMany') {
            if (anyArgs.skip === undefined) {
              anyArgs.skip = 0;
            }
            if (anyArgs.orderBy === undefined) {
              anyArgs.orderBy = {
                updated_at: 'desc',
              };
            }
          }

          const result = await query(args);

          // Timezone conversion for results
          if (operation.startsWith('find') && result) {
            const convertDates = (item: any) => {
              if (!item || typeof item !== 'object') return;
              if (item.created_at) item.created_at = toTimezone(item.created_at, tz);
              if (item.updated_at) item.updated_at = toTimezone(item.updated_at, tz);
              if (item.expires_at) item.expires_at = toTimezone(item.expires_at, tz);

              for (const key in item) {
                if (Object.prototype.hasOwnProperty.call(item, key) && key.toLowerCase().includes('date')) {
                  item[key] = toTimezone(item[key], tz);
                }
              }
            };

            if (Array.isArray(result)) {
              result.forEach(convertDates);
            } else {
              convertDates(result);
            }
          }

          return result;
        },
      },
    },
  });
});
